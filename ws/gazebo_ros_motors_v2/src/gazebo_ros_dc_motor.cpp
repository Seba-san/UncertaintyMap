...
void GazeboRosMotor::Load ( physics::ModelPtr _parent, sdf::ElementPtr _sdf ) {
    ...
    // New part, define axis for rotation
     gazebo_ros_->getParameter<double> ( axis_, "axis", 2 ); // 0-x, 1-y, 2-z
    ROS_INFO_NAMED(plugin_name_, "axis_ (0-x, 1-y, 2-z) = %f", axis_);
    ...
    // gazebo_ros_->getParameter<double> ( joint_damping_, "joint_damping", 0.001 ); // Friccion!
    // gazebo_ros_->getParameter<double> ( joint_friction_, "joint_friction", 0.1 ); // Friccion!
...
void GazeboRosMotor::paramServerUpdate()
{
  if (notify_server_) {
    ...
    // current_config_.joint_friction       = joint_friction_;
    // current_config_.joint_damping       = joint_damping_;
...
void GazeboRosMotor::motorModelUpdate(double dt, double output_shaft_omega, double actual_load_torque) {
    ...
    //#### Modificado por Sebasan
    // Asumo que la corriente no tiene dinamica
    double i_t =(V-Km*internal_omega_)/R;
    //#### fin Modificado por Sebasan
...
void GazeboRosMotor::UpdateChild() {
    ...
    if ( seconds_since_last_update > update_period_ ) {
        ...
        //double current_noisy_output_speed = current_output_speed;
