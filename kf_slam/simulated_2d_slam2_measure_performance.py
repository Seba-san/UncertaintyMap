...
class SLAM2D_simulated(Slam2d_base):
    ...
    def __init__(self,kalman_object=KF(),control_signal=np.array([[0,0]]),debug=False,parameters=[]):
        ...
        self.FOV=4.0 #m TODO: make a function or object with FOV methods  
        self.new_data=False  
    ...
    def fix_parameters(self,parameters):
        ...
        self.kf.Q_=np.array([[1,0],[0,1]],dtype='f').dot(parameters[2]+0.01) # ojo aca! $1
...
class Simulator_manager:
    ...
    def __init__(self,debug=0):
       ...
       #
        ...
        if rospy.get_param('UF'):
            pass
            #self.marker=RvizMarkerPublisher()
            #time.sleep(5)

        locals_dict = {'self': self}
        cProfile.runctx('self.instances_slam2d_simulated2()', globals(), locals_dict)
       
        #cProfile.run('self.instances_slam2d_simulated2()', 'profile_results')
        #self.instances_slam2d_simulated2()
    ...
    def make_u_controls_extended(self,u_signal):
        ...
        return u_total
    def instances_slam2d_simulated2(self):
        """
        ...
        parameters = [P0,R,Q,new_landmark,sigma_max,fov,cell_size] #P0 R, Q, new_landmark,sigma_max,fov,cell_size
        ...
        u_extra,flag=self.get_aditional_u_control()
        if not flag:
